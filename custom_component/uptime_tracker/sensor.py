...
class LastOfflineDurationSensor(UptimeTrackerSensor):
    ...
    def _update_state(self):
        if not self._initialized:
            last_offline_state = self._hass.states.get('sensor.uptime_tracker_last_offline')
            if last_offline_state and last_offline_state.state:
                last_offline_time = datetime.fromisoformat(last_offline_state.state)
                current_time = datetime.now()
                duration_seconds = (current_time - last_offline_time).total_seconds()
...
class LastOfflineSensor(UptimeTrackerSensor):
    def __init__(self, hass, name):
        super().__init__(hass, name, 'Last Offline')

    def _update_state(self):
        self._state = datetime.now().isoformat()
        self._hass.states.async_set('sensor.uptime_tracker_last_offline', self._state, {"friendly_name": "Last Offline"})
        _LOGGER.debug(f"{self._name} state updated to: {self._state}")